...
public class ShopInterface extends ComponentCentered implements ItemGroup {
    ...
    public ShopInterface(ScriptCore core, String expectedTitle) {
        super(core);
        this.expectedTitle = expectedTitle == null ? "" : expectedTitle.trim();
    }
    ...
    public UIResult<Integer> getSelectedAmount() {
        ...
        List<Rectangle> bottomTextButtonsRed = core.getImageAnalyzer().findContainers(
                bounds,
        ...
        String buttonText = core.getOCR()
                .getText(Font.STANDARD_FONT, bottomTextButtonsRed.get(0), ColorUtils.ORANGE_UI_TEXT)
                .replaceAll("[^0-9]", "");
    ...
    public boolean setSelectedAmount(int amount) {
        ...
        List<Rectangle> bottomTextButtons = core.getImageAnalyzer().findContainers(
                bounds,
        ...
        for (Rectangle button : bottomTextButtons) {
            String buttonText = core.getOCR()
                    .getText(Font.STANDARD_FONT, button, ColorUtils.ORANGE_UI_TEXT)
                    .replaceAll("[^0-9]", "");
            if (buttonText.isEmpty()) {
                continue;
            }
