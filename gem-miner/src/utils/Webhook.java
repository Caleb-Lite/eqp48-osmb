...
public class Webhook {
  ...
  private List<String> locationOptions = null;
  private Supplier<String> currentLocationSupplier = null;
  private Consumer<String> locationSaveConsumer = null;
  ...
  public void showDialogWithLocation(List<String> locations, Supplier<String> currentLocation, Consumer<String> onSave) {
    this.locationOptions = locations;
    this.currentLocationSupplier = currentLocation;
    this.locationSaveConsumer = onSave;
    showDialog();
  }
  ...
  public void showDialog() {
    ...
    SwingUtilities.invokeLater(() -> {
      ...
      final JComboBox<String>[] locationComboRef = new JComboBox[]{null};

      int row = 0;
      if (locationOptions != null && !locationOptions.isEmpty()) {
        JComboBox<String> combo = new JComboBox<>(locationOptions.toArray(new String[0]));
        locationComboRef[0] = combo;
        String current = currentLocationSupplier != null ? currentLocationSupplier.get() : null;
        if (current != null) {
          combo.setSelectedItem(current);
        }
        gbc.gridx = 0; gbc.gridy = row; dialog.add(new JLabel("Mining location:"), gbc);
        gbc.gridx = 1; gbc.gridy = row; dialog.add(combo, gbc);
        row++;
      }
      ...
      gbc.gridx = 0; gbc.gridy = row; dialog.add(new JLabel("Webhook URL:"), gbc);
      gbc.gridx = 1; gbc.gridy = row; dialog.add(urlField, gbc);
      row++;

      gbc.gridx = 0; gbc.gridy = row; dialog.add(new JLabel("Interval (minutes):"), gbc);
      gbc.gridx = 1; gbc.gridy = row; dialog.add(intervalField, gbc);
      row++;
      ...
      saveBtn.addActionListener(event -> {
        ...
        if (locationSaveConsumer != null && locationComboRef[0] != null) {
          String selection = (String) locationComboRef[0].getSelectedItem();
          locationSaveConsumer.accept(selection != null ? selection : "");
        }
